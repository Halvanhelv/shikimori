...
## Простыми словами
Было одно множество ачивок, стало другое — и нужно понять, что именно изменилось: что добавить, что убрать, что обновить. Это как сравнить старый и новый список покупок: одни товары появились, другие вычеркнули, у третьих поменялось количество. Код сравнивает наборы по «удостоверению личности» элемента (составному ключу) и выдаёт ровно три кучки изменений.
...
## 🆚 Классический Rails
**Обычно (наивный подход):** при «было → стало» тупо стирают всё старое и записывают всё новое (`delete_all` + `insert`), или гоняют вложенные циклы для сравнения. Это лишние записи в БД, лишние перерисовки UI и потеря информации о том, что конкретно изменилось.
**Здесь:** оба набора превращают в хеш `{составной_ключ => элемент}`, и через операции над множествами (`Map.drop`, `MapSet.difference`) получают ровно три набора — added/removed/updated, применяя только реальную разницу.

## Бенефиты
- Минимальные изменения: трогаем только то, что реально поменялось.
- Составной ключ идентичности чётко отделяет «новый объект» от «обновлённого значения».
- Чистый, декларативный код без вложенных циклов сравнения.
- Универсально для любой синхронизации состояний (теги, права, подписки, индексы).
