...
## Простыми словами
Когда надо разово массово переразложить данные (например, слить дубли жанров), это делают не в обычной Rails-миграции, а в отдельном классе. Миграции отвечают за структуру таблиц (добавить колонку), а такие классы — за содержимое. Это как разница между «переставить полки в шкафу» и «перебрать и разложить по ним вещи».
...
## 🆚 Классический Rails
**Обычно:** сложные перекладки данных пишут прямо внутри файла миграции (`db/migrate`) — а такую миграцию нельзя перезапустить, тяжело тестировать и она намертво «зашита» в историю схемы.
**Здесь:** логика данных вынесена в обычный callable-объект в `app/migrators` и запускается вручную из консоли/задачи; схема и данные разделены.

## Бенефиты
- Перекладку можно перезапустить, отладить и протестировать как обычный код.
- Миграции остаются чистыми — только про структуру БД.
- Сложные приёмы (свап через `TEMP_ID`, транзакции) живут в нормальном классе, а не в одноразовом скрипте миграции.
