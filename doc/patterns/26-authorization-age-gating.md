...
## Простыми словами
Новенький пользователь получает права не сразу, а постепенно: сначала может комментировать, через сутки — писать отзывы, через неделю — темы на форуме. Это как испытательный срок на работе: доверие растёт со временем, а не выдаётся в первый день. Спамеру невыгодно ждать неделю ради одного аккаунта.
...
## 🆚 Классический Rails
**Обычно:** права проверяют разрозненными `if current_user.admin?` прямо в контроллерах и вьюхах; логика «кто что может» размазана по всему приложению и дублируется.
**Здесь:** единый класс `Ability` (на `cancancan`) склеивает права из ролевых модулей через `merge`, а доступ к фичам ещё и зависит от возраста аккаунта (`day_registered?`, `week_registered?`).

## Бенефиты
- Все правила доступа в одном месте, а не россыпью `if` по коду.
- Права собираются из переиспользуемых ролевых модулей — легко строить иерархию admin > moderator > user.
- Time-gating даёт мягкий анти-спам без жёстких банов.
- Логику авторизации удобно тестировать изолированно.
