...
## Простыми словами
Две вещи. Первая: нужно загрузить два независимых куска данных — делаем это одновременно, а не по очереди, и ждём, пока оба будут готовы (общее время = самый медленный из двух, а не сумма). Вторая: вместо лестницы из `if/else` действие выбирается «по форме» входящего запроса — как сортировка писем по разным ящикам в зависимости от того, что на конверте написано.
...
## 🆚 Классический Rails
**Обычно (наивный подход):** независимые запросы к API/БД делают последовательно (общее время = сумма всех), а ветвление действия пишут одной длинной лестницей `if/elsif/else` или `case`, где условия легко перепутать и забыть ветку.
**Здесь:** два источника грузятся параллельно через `Task.async/await` (время = max, а не sum), а выбор действия — это несколько определений функции с разными образцами и guard'ами (`when status in [...]`); рантайм сам выбирает подходящее, тела ветвлений нет.

## Бенефиты
- Параллельная загрузка независимых источников: суммарная задержка = самый медленный, а не сумма.
- Каждая ветка логики — отдельное объявление с явным условием на входе: читаемо и расширяемо.
- Нет вложенных `if` в теле — бизнес-правило выражено самой формой образца.
- Пропущенный случай заметен сразу (нет подходящего образца → явная ошибка).
